<template>
  <form class="quiz-question">
    <section class="question-card quiz-card-content">
      ...
      <div class="answer">
        <input class="check-itself"
               type="radio"
               name="answer"
               id="answer_1" />
        <label @click="anserChoose = true"
               for="answer_1">3±1</label>
        <input class="check-itself"
               type="radio"
               name="answer"
               id="answer_2" />
        <label @click="anserChoose = true"
               for="answer_2">13±2</label>
        <input class="check-itself"
               type="radio"
               name="answer"
               id="answer_3" />
        <label @click="anserChoose = true"
               for="answer_3">7±2</label>
        <input class="check-itself"
               type="radio"
               name="answer"
               id="answer_4" />
        <label @click="anserChoose = true"
               for="answer_4">19±2</label>
      </div>
    </section>
    ...
    <quizPopover v-show="answer"
                 @click="$emit('answer', {componentId: 'quizQuestionAbbreviation', quiz:1,missionTimeData:{ level:2, section:1,nextLevel:2,nextSection:2 }})">
